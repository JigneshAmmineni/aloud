...
- **Branching model:** `main` is the GitHub default and the integration "workbench" — feature branches PR into it (CI + Claude review gate). `prod` is the production pointer: it only ever moves to a **verified-deployed** commit (the deploy workflow moves it after the health check; rollback moves it backward). Promotion to prod is always the user's explicit call. `prototype-gemini-live` preserves the original Gemini Live prototype; the retired design docs (SDD.md, SDD-v2.md) live in git history at commit `a84df1b`.
...
- **When a review lands, the summary leads.** First output a concise summary of the findings with an assessment (real vs. churn; what will be fixed vs. pushed back on), then proceed to fix autonomously in the same turn — the user reads along and vetoes afterward. The unpushed commit is the veto window. Never fix-then-summarize; never summarize-then-wait.

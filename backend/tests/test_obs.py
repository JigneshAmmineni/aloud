...
def test_measurement_hands_off_to_the_recorder_buffer():
    """FR-47 wiring: at first audio the observer hands its e2e ms into the
    recorder's per-turn buffer (record_measurement, which captures the
    turn number with the measurement) — the LOOP writes the row at turn
    end, not the observer."""
    from unittest.mock import MagicMock
    ...
    recorder.record_measurement.assert_called_once()
    assert recorder.record_measurement.call_args.args[0] >= 0


def test_no_end_of_speech_measures_nothing():
    """The greeting turn (bot speaks with no preceding end-of-user-speech)
    measures nothing, so the buffer never sees a measurement and no row can
    be fabricated — those numbers feed FR-37's p50/p95 and the NFR-1
    breach count."""
    from unittest.mock import MagicMock
    ...
    asyncio.run(run())
    recorder.record_measurement.assert_not_called()
